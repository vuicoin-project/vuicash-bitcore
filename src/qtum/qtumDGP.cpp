...
bool VuiCashDGP::checkLimitSchedule(const std::vector<uint32_t>& defaultData, const std::vector<uint32_t>& checkData){
...
dev::eth::EVMSchedule VuiCashDGP::getGasSchedule(unsigned int blockHeight){
    clear();
...
uint64_t VuiCashDGP::getUint64FromDGP(unsigned int blockHeight, const dev::Address& contract, std::vector<unsigned char> data){
    uint64_t value = 0;
...
uint32_t VuiCashDGP::getBlockSize(unsigned int blockHeight){
    clear();
...
uint64_t VuiCashDGP::getMinGasPrice(unsigned int blockHeight){
    clear();
...
uint64_t VuiCashDGP::getBlockGasLimit(unsigned int blockHeight){
    clear();
...
bool VuiCashDGP::initStorages(const dev::Address& addr, unsigned int blockHeight, std::vector<unsigned char> data){
    initStorageDGP(addr);
...
void VuiCashDGP::initStorageDGP(const dev::Address& addr){
    storageDGP = state->storage(addr);
}

void VuiCashDGP::initStorageTemplate(const dev::Address& addr){
    storageTemplate = state->storage(addr);
}

void VuiCashDGP::initDataTemplate(const dev::Address& addr, std::vector<unsigned char>& data){
    dataTemplate = CallContract(addr, data)[0].execRes.output;
}

void VuiCashDGP::createParamsInstance(){
...
dev::Address VuiCashDGP::getAddressForBlock(unsigned int blockHeight){
...
void VuiCashDGP::parseStorageScheduleContract(std::vector<uint32_t>& uint32Values){
...
void VuiCashDGP::parseDataScheduleContract(std::vector<uint32_t>& uint32Values){
...
void VuiCashDGP::parseStorageOneUint64(uint64_t& value){
...
void VuiCashDGP::parseDataOneUint64(uint64_t& value){
    if(dataTemplate.size() == 32){
        value = uint64_t(dev::u256(dev::h256(dataTemplate)));
    }
}

dev::eth::EVMSchedule VuiCashDGP::createEVMSchedule(){
    dev::eth::EVMSchedule schedule = dev::eth::EIP158Schedule;
