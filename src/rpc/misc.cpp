...
UniValue getdgpinfo(const JSONRPCRequest& request)
{
    ...
    obj.push_back(Pair("maxblocksize", (uint64_t)vuicashDGP.getBlockSize(chainActive.Height())));
    obj.push_back(Pair("mingasprice", (uint64_t)vuicashDGP.getMinGasPrice(chainActive.Height())));
    obj.push_back(Pair("blockgaslimit", (uint64_t)vuicashDGP.getBlockGasLimit(chainActive.Height())));
...
UniValue getinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total vuicash balance of the wallet\n"
            "  \"stake\": xxxxxxx,           (numeric) the total vuicash stake balance of the wallet\n"
...
UniValue validateaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw runtime_error(
            ...
            "\nReturn information about the given vuicash address.\n"
            "\nArguments:\n"
            "1. \"address\"     (string, required) The vuicash address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"address\", (string) The vuicash address validated\n"
...
UniValue createmultisig(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 2)
    {
        string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are vuicash addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) vuicash address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 3)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The vuicash address to use for the signature.\n"
...
UniValue echo(const JSONRPCRequest& request)
{
    if (request.fHelp)
        throw runtime_error(
            ...
            "vuicash-cli and the GUI. There is no server-side difference."
        );
...
{ //  category              name                      actor (function)         okSafeMode
  //  --------------------- ------------------------  -----------------------  ----------
    ...
    /////////////////////////////////////////////////////////////////////////////////////////////////////////////// // vuicash
    { "util",               "getaddresstxids",        &getaddresstxids,        true,  {"addresses"} },
    { "util",               "getaddressdeltas",       &getaddressdeltas,       true,  {"addresses"} },
    { "util",               "getaddressbalance",      &getaddressbalance,      true,  {"addresses"} },
    { "util",               "getaddressutxos",        &getaddressutxos,        true,  {"addresses"} },
    { "util",               "getaddressmempool",      &getaddressmempool,      true,  {"addresses"} },
    { "util",               "getblockhashes",         &getblockhashes,         true,  {"high","low","options"} },
    { "util",               "getspentinfo",           &getspentinfo,           true,  {"argument"} },
    ///////////////////////////////////////////////////////////////////////////////////////////////////////////////
